...
private:
	void ReadObjCore(const std::string filename);
	void WriteObjCore(const std::string filename);
	...
	void CalculateTriangleNormal(
		...
		Eigen::Vector3d &out);
	void CalculateAdjacentVerticesPerVertex();
	void CalculateAdjacentFacetsPerVertex();

private:
	std::vector<Eigen::Vector3d> Vertices;
	std::vector<int> Facets;
	std::vector<Eigen::Vector3d> VertexNormals;
	std::vector<Eigen::Vector3d> FacetNormals;

	std::vector<std::vector<int> *> adjacentVerticesPerVertex;
	std::vector<std::vector<int> *> adjacentFacetsPerVertex;
};
...
static void ReadObj(
	...
	std::vector<Eigen::Vector3d> &normals = std::vector<Eigen::Vector3d>(),
	std::vector<int> &facets = std::vector<int>()){
	(new Mesh())->ReadObj(filename, points, normals, facets);
}
...
static void WriteObj(
	...
	const std::vector<Eigen::Vector3d> &normals = std::vector<Eigen::Vector3d>(),
	const std::vector<int> &facets = std::vector<int>()){
	(new Mesh())->WriteObj(filename, points, normals, facets);
}
...
static void WriteObj(
	...
	for (int i = 0; i < (int)points.size(); ++i){
		ofs << "v "
			...
			<< (float)colors[i][0] << " "
			<< (float)colors[i][1] << " "
			<< (float)colors[i][2] << endl;
	}

	ofs << "# " << points.size() << " vertices" << endl;
	ofs.close();
}
